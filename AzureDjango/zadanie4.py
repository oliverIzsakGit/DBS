...
import os
from AzureDjango import models
from django.db.models import Max,Value,TextField,Count
...
def objs(request,pl_id):
    
    
         
            
    posts =models.GameObjectives.objects.using('xizsakdbs').select_related('match_player_detail_id_1').select_related('match_player_detail_id_1__player_id').\
    select_related('match_player_detail_id_1__hero_id')\
    .select_related('match_player_detail_id_1__match_id')\
    .values('match_player_detail_id_1__player_id','match_player_detail_id_1__match_id','match_player_detail_id_1__player_id__nick')\
    .filter(match_player_detail_id_1__player_id=pl_id).\
    annotate(player_nick=Coalesce('match_player_detail_id_1__player_id__nick',Cast(Value('Unknown'), TextField())))\
    .annotate(hero_action=Coalesce('subtype',Cast(Value('NO_ACTION'), TextField())))\
    .values('match_player_detail_id_1__player_id','match_player_detail_id_1__match_id','match_player_detail_id_1__hero_id__localized_name','hero_action', 'player_nick')\
    .annotate(count=Greatest(Count('hero_action'),1))\
    .values('match_player_detail_id_1__player_id','player_nick','match_player_detail_id_1__hero_id__localized_name','match_player_detail_id_1__match_id','hero_action','count')
    ...
    for row in posts:
                if temp != row["match_player_detail_id_1__player_id"]:
                    temp = row["match_player_detail_id_1__player_id"]
                    repsonse = {"id": row["match_player_detail_id_1__player_id"], "player_nick": row["player_nick"], "matches": []}
                    repsonse["matches"].append(
                        {"match_id": row["match_player_detail_id_1__match_id"], "hero_localized_name": row["match_player_detail_id_1__hero_id__localized_name"],
                         "actions": []})
                    repsonse["matches"][i]["actions"].append({"hero_action": row["hero_action"], "count": row["count"]})
                    match = row["match_player_detail_id_1__match_id"]


                elif match != row["match_player_detail_id_1__match_id"]:
                    i += 1
                    match = row["match_player_detail_id_1__match_id"]
                    repsonse["matches"].append(
                        {"match_id": row["match_player_detail_id_1__match_id"], "hero_localized_name": row["match_player_detail_id_1__hero_id__localized_name"],
    ...
    return JsonResponse(repsonse,safe=True)



def abilities(request,pl_id):
    if request.method !='GET':
        return JsonResponse({"error! Wrong method"},status=405)
    if type(pl_id)!=int:
        return JsonResponse({"error! Player id is not int"},status=405)
            
    query= models.AbilityUpgrades.objects.using('xizsakdbs').select_related('ability_id').select_related('match_player_detail_id').select_related('match_player_detail_id__hero_id').select_related('match_player_detail_id__match_id').select_related('match_player_detail_id__player_id').\
        filter(match_player_detail_id__player_id=pl_id).values('match_player_detail_id__player_id','ability_id','match_player_detail_id__match_id','match_player_detail_id__hero_id').annotate(upgrade_level=Max('level')).annotate(count=Count('ability_id')).annotate(player_nick=Coalesce('match_player_detail_id__player_id__nick',Value('Unknown'))).\
        order_by('match_player_detail_id__match_id','ability_id__name').values('match_player_detail_id__player_id','match_player_detail_id__player_id__nick','match_player_detail_id__hero_id__localized_name','match_player_detail_id__match_id','ability_id__name','ability_id','upgrade_level','count') 
    ...
    for row in query:
                if temp != row["match_player_detail_id__player_id"]:
                    temp = row["match_player_detail_id__player_id"]
                    repsonse = {"id": row["match_player_detail_id__player_id"], "player_nick": row["match_player_detail_id__player_id__nick"], "matches": []}
                    repsonse["matches"].append(
                        {"match_id": row["match_player_detail_id__match_id"], "hero_localized_name": row["match_player_detail_id__hero_id__localized_name"],
                         "abilities": []})
                    repsonse["matches"][i]["abilities"].append({"ability_name": row["ability_id__name"], "count": row["count"], "upgrade_level": row["upgrade_level"]})
                    match = row["match_player_detail_id__match_id"]


                elif match != row["match_player_detail_id__match_id"]:
                    i += 1
                    match = row["match_player_detail_id__match_id"]
                    repsonse["matches"].append(
                        {"match_id": row["match_player_detail_id__match_id"], "hero_localized_name": row["match_player_detail_id__hero_id__localized_name"],
    ...
    return JsonResponse(repsonse,safe=False) 
